...
\begin{tabular}{llccccc}
\toprule
Variant @ $40$k & M. & \textsc{Sp.} & \textsc{Obj.} & \textsc{Goal} & \textsc{Long} & Avg \\
\midrule
\multicolumn{7}{l}{\emph{(a) Mechanism vs.\ weights --- machine A}}\\
Uniform & A & 73.0 & 95.5 & 38.5 & 40.5 & 61.9 \\
Uniform-interleaved & A & 97.0 & 96.0 & 85.5 & 70.5 & 87.25 \\
\tds{} (ours) & A & 97.5 & 98.5 & 92.0 & 71.0 & \textbf{89.75} \\
\midrule
\multicolumn{7}{l}{\emph{(b) Free vs.\ oracle difficulty --- machine B}}\\
Uniform-interleaved & B & 95.0 & 93.5 & 83.5 & 67.5 & 84.9 \\
\tds{} (ours) & B & 97.0 & 99.5 & 87.0 & 64.0 & 86.9 \\
Measured-difficulty & B & 94.0 & 98.0 & 90.0 & 69.5 & 87.9 \\
\bottomrule
\end{tabular}
\caption{\textbf{Ablations} (SR \%, $40$k budget; ``M.''\ is the evaluation machine, comparisons are within-block / same-machine). \emph{Block (a):} the \emph{uniform-interleaved} control (our two-level sampler with flat weights) decomposes the headline gain --- it already reaches $87.25$, so the bulk of the improvement over naive uniform ($61.9$) comes from the sampler's fine-grained batch decorrelation, while the transition-density weighting adds a further $+2.5$ on average (concentrated on the up-weighted \textsc{Goal} suite, $+6.5$). \emph{Block (b):} replacing the free transition-density difficulty with an \emph{oracle} measured difficulty (sampling weights from the model's own per-task SR) gains only $+1.0$ over \tds{} ($87.9$ vs.\ $86.9$), i.e.\ the free, model-free signal recovers nearly all of the costly one; both improve over flat by only $+2$--$3$, so the difficulty-weighting ceiling on LIBERO is modest. The $\sim\!3$\,pt A--B offset (flat $87.25$ vs.\ $84.9$; \tds{} $89.75$ vs.\ $86.9$) is consistent across rows. \textbf{[TODO: inverted-weights and no-floor rows; eval measured-difficulty on machine A to unify; $\ge 2$ seeds to tighten the $+2.5$, near the $20$-trial noise floor.]}}
\label{tab:ablation}
\end{table}

\paragraph{Decomposing the gain.} The uniform-interleaved control is the most informative ablation: it shows that the headline improvement over the naive uniform baseline has two distinct sources. First, a \emph{mechanism} effect---replacing per-suite episode streaming with fine-grained per-episode interleaving decorrelates the training batch (from $\sim\!4$ to $\sim\!64$ distinct episodes per batch of $64$) and accounts for the majority of the gain ($61.9\!\rightarrow\!87.25$, $+25.4$ same-machine), with the largest lift on the hard suites that the coarse sampler under-mixed (\textsc{Goal} $+47$, \textsc{Long} $+30$). Second, the \emph{transition-density weighting} adds a smaller, targeted increment on top of the well-mixed sampler ($87.25\!\rightarrow\!89.75$, $+2.5$ on average, $+6.5$ on the up-weighted \textsc{Goal} suite). We report this decomposition transparently: the practical recommendation is the full pipeline (interleaving $+$ density weighting), but the novel, model-free component---transition-density reweighting---should be credited with the incremental gain over a properly-mixed uniform baseline, not the full margin over the naive one. \textbf{[TODO: $\ge 2$ seeds to confirm the $+2.5$ weighting effect, which is currently near the $20$-trial noise floor.]}

\paragraph{Free difficulty vs.\ the oracle.} The central claim of \tds{} is that difficulty can be read off the demonstrations \emph{without} a model. Block (b) of Table~\ref{tab:ablation} tests this directly by swapping our free transition-density weights for an \emph{oracle}: sampling weights derived from the trained model's own measured per-task success rate (the ``ask-the-model'' upper bound, which costs a full train-then-evaluate cycle). On the same machine, the oracle reaches $87.9$ versus \tds{}'s $86.9$---a gap of only $+1.0$, within the $20$-trial standard error. The free, offline transition-density signal thus recovers essentially all of the benefit of the expensive measured difficulty, validating the core premise that the cost of difficulty estimation can be eliminated. Notably both lie only $+2$--$3$ above the well-mixed flat sampler, so on LIBERO the headroom for \emph{any} difficulty weighting is small once batch mixing is fixed; \tds{} captures that headroom at zero additional cost.
