...
\bottomrule
\end{tabular}
\caption{\textbf{Ablations} (SR \%, $40$k budget). Expected readings: measured-difficulty $\approx$ \tds{} (free difficulty matches the costly model-coupled one); uniform-interleaved $\approx$ uniform (the gain is from the weights, not the mechanism); inverted hurts \textsc{Goal}/\textsc{Long} (direction carries information); no-floor degrades \textsc{Spatial}/\textsc{Object} (the floor prevents forgetting). \textbf{[TODO: fill once the control runs complete.]}}
\label{tab:ablation}
\end{table}
