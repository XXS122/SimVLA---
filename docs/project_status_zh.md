...
**卡点**:第二轮 Flow-DPO 训练(实验 14)+ 5 checkpoint 自动筛选(实验 15)尚未产出数字。

**下一步(按顺序)**:
1. **[你]** 用 `SIMVLA_ENV=base` 跑 `screen_checkpoints.sh`,把最终打印的汇总表(5 个 checkpoint 的成功率)发我。
2. **[我]** 根据 5 个数字判断:
   - 若有 ≥65% 的 → 对该 checkpoint 补测到 50 回合确认,若稳定在 70% 附近 → Flow-DPO 第一轮验证成功,进入"新 ckpt 上服务端重新采集 → 第二轮 DPO"的在线迭代循环;
   - 若全部仍明显偏低(如 <58%)→ 说明问题不只是标签噪声,需要进一步排查(可能是数据量本身不够、或需要更换损失权重方案);
   - 若全部却仍然过拟合(`implicit_acc` 早早到 1.0)→ 数据量(200 对)相对模型容量仍然偏少,建议扩大采集(实验 6 从 100 回合扩到 300+,双机并行)再训。
3. **[可选,待办已久]** 把 libero_10 十个任务的 TDS 分数发我 → 做"模型早期不确定性 vs 人工 TDS"相关分析(实验 4 的副产品,免费出图)。
